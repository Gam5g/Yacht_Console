...
public class YachtDisplay {
	...
	public YachtDisplay() {
		...
		System.out.println("★☆★☆★☆★☆★☆★☆★☆★☆★☆★☆★☆★☆★☆★☆★☆★☆★☆");
		System.out.print("플레이어의 수를 입력해 주세요 : ");
	...
	private void _Turn() {
		...
		} while (!nowPlayer.SetScore(sortedDiceData, resPut));

		System.out.println();

	}

	// 주사위를 다루는 클래스를 분리한다면 UI 작업할 때 편해질 것 같지만
	// 현재는 그럴 필요가 적어 보여 그대로 둠
	...
	private boolean _Keep(List<Integer> dices) {
		...
		while (keepCount < 5) {
			...
			if (num >= dices.size() || num < 0 || dices.get(num) == -1) {
