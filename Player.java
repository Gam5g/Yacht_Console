...
public class Player {
    ...
    // NumSum을 6에서 12에 저장하는 것으로 변경하여 그 이후의 점수가 한 인덱스 당겨졌다.
    ...
    public boolean SetScore(int[] data, String res) {
        ...
        // One~Six의 보너스 점수를 얻지 못했을 때 출력하는 메시지
      	if (!_zoneCheck[12]) {
        	...
        	if (numSum >= 63) {
            	System.out.println("\n★☆★☆★☆★☆★☆★☆★☆★☆★☆★☆★☆★☆★☆★☆★☆★☆★☆");
            	GameManager.PrintFontCyanln("님의 One~Six의 총합이 63을 넘어섰으므로 추가 점수 35점을 획득했습니다.");
            	System.out.print("★☆★☆★☆★☆★☆★☆★☆★☆★☆★☆★☆★☆★☆★☆★☆★☆★☆");
            	_zoneScore[12] = 35;
            	_zoneCheck[12] = true;
        	}
    ...
    // static 함수로 만들어 놓았지만 다른 클래스로 분리하거나 GameManager 클래스에 넣어도 될 것 같다.
    ...
    // _playerName을 인수로 전달받으면 static 함수로 바꿀 수 있을 것으로 보이지만(player에서 분리하는 것이 객체지향적일 것 같음)
    // 인수가 많아짐에 따른 오버헤드 등을 고려했을 때 이대로 두는 것도 괜찮을 것 같다.
